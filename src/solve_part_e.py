...
def plot_sample_fit(
    ...
    ax1.plot(
        x_plot,
        weight_s * scale_factor * pdf_s,
        label="Signal density " "component fit",
    )
    ax1.plot(
        x_plot,
        weight_b * scale_factor * pdf_b,
        label="Background density " "component fit",
    )
    ax1.plot(x_plot, scale_factor * pdf_sb, label="Total density fit")
    ax1.set(
        ylabel="Events",
    ...
    ax3.plot(
        norm.pdf(x_pull_plot, loc=0, scale=1),
        x_pull_plot,
        c="green",
        alpha=0.5,
    )
    ...
    ax1.text(
        x=5.02,
        y=1800,
        s="Chi sq./d.o.f = {:.3f}".format(chi2_score / dof),
        fontsize=18,
    )
...
plot_sample_fit(*mi.values, "part_e_plot.png", nh, xe)
